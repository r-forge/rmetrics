************************************************************************
* R port by Yohan Chalabi / 2008-01-29
* - added IMPLICIT NONE statement in all subroutines and functions
* - removed common block /STAT/ and added array STAT as an argument in
*   subroutines :
*   VARIABLES IN ARRAY STAT (STATISTICS) :
*    IO  STAT(1) NRES  NUMBER OF RESTARTS.
*    IO  STAT(2) NDEC  NUMBER OF MATRIX DECOMPOSITION.
*    IO  STAT(3) NREM  NUMBER OF CONSTRAINT DELETIONS.
*    IO  STAT(4) NADD  NUMBER OF CONSTRAINT ADDITIONS.
*    IO  STAT(5) NIT  NUMBER OF ITERATIONS.
*    IO  STAT(6) NFV  NUMBER OF FUNCTION EVALUATIONS.
*    IO  STAT(7) NFG  NUMBER OF GRADIENT EVALUATIONS.
*    IO  STAT(8) NFH  NUMBER OF HESSIAN EVALUATIONS.
* - changed WRITE statement to DBLEPR for R compatibitly
* - added RCHKUSR() to allow interrupts from R
************************************************************************

************************************************************************
* Copyright:
*
* Subroutines PMIN, PBUN, PNEW, PVAR, Copyright ACM, 2001. The original
...
* License: 
*
* This library (with exception of PMIN, PBUN, PNEW, PVAR) is a free 
...
* the Availability note, and "Used by permission." Permission to modify
...
*
     ...
     +                RPF,CMAX,GMAX,F,MIT,MFV,MET,MEC,IPRNT,ITERM, STAT)
      IMPLICIT NONE
...
*
   ...
   40 CONTINUE
      LDS=LD
      CALL PF1F01(NF,X,GF,GF,FF,F,KD,LD,IEXT, STAT)
      LD=LDS
      CALL PC1F01(NF,NC,X,FC,CF,CL,CU,IC,GC,CG,CMAX,KD,LD)
      CF(NC+1)=F
c      IF (ABS(IPRNT).GT.1)
c     + WRITE (6,'(1X,''STAT(5)='',I4,2X,''STAT(6)='',I4,2X,''STAT(7)='',I4,2X,
c     + ''F='',G12.6,2X,''C='',E7.1,2X,''G='',E7.1)')
c     +  STAT(5),STAT(6),STAT(7),F,CMAX,GMAX
...
*
      ...
      ELSE
          NTESX = 0
      END IF
   50 IF (STAT(5).GE.MIT) THEN
          ITERM = 11
          GO TO 80
      END IF
      IF (STAT(6).GE.MFV) THEN
          ITERM = 12
          GO TO 80
      END IF
      ITERM = 0
...
11170 CONTINUE
     ...
     & TOLS,KD,LD,STAT(5),KIT,NRED,MRED,MAXST,IEST,INITS,ITERS,KTERS,
     & MES,ISYS)
...
11172 CONTINUE
      ...
      LDS=LD
      CALL PF1F01(NF,X,GF,G,FF,F,KD,LD,IEXT, STAT)
      LD=LDS
      CALL PC1F01(NF,NC,X,FC,CF,CL,CU,IC,GC,CG,CMAX,KD,LD)
      CF(NC+1)=F
      CALL PP0AF8(NF,N,NC,CF,IC,ICA,CL,CU,CZ,RPF,FC,F)
...
*     VARIABLE METRIC UPDATE
*
      CALL PUDBG1(N,H,G,S,XO,GO,R,PO,STAT(5),KIT,ITERH,MET,MET1,MEC)
...
c     + STAT(5),STAT(6),STAT(7),F,CMAX,GMAX,ITERM
c      IF (IPRNT.LT.0)
...
*
      SUBROUTINE PF1F01(NF,X,GF,G,FF,F,KD,LD,IEXT, STAT)
      IMPLICIT NONE
...
*
     ...
     & CR,CZ,G,GO,S,MFP,KBF,KBC,ETA9,EPS7,EPS9,UMAX,GMAX,N,ITERL, STAT)
      IMPLICIT NONE
...
c      INTEGER NRES,NDEC,NREM,NADD,NIT,NFV,NFG,NFH
      INTEGER STAT(8)
c      COMMON /STAT/ NRES,NDEC,NREM,NADD,NIT,NFV,NFG,NFH
      CON=ETA9
*
...
C      IF (NF.GT.N) CALL PLSETC(NF,NC,X,XO,CF,IC,CG,S)
      ...
      CALL PLADB0(NF,N,ICA,CG,CR,CZ,S,EPS7,GMAX,UMAX,INEW,STAT(4),IER)
...
*     CONSTRAINT DELETION
*
      CALL PLRMB0(NF,N,ICA,CG,CR,CZ,GO,S,IOLD,KREM,STAT(3),IER)
      KC=ICA(NF-N+1)
      IF (KC.GT.0) THEN
      IC(KC)=-IC(KC)
      ELSE
...
C
      ...
      CZ(J)=CZ(J+1)
    6 CONTINUE
      CALL PLRMF0(NF,NC,IX,IC,ICA,CR,IC,G,N,IOLD,KREM,IER, STAT)
      NCR=NCR-NCA
      NCA=NCA-1
      JOLD=IOLD
      JNEW=0
...
*         AFTER CONSTRAINT DELETION.
*
      SUBROUTINE PLRMF0(NF,NC,IX,IA,IAA,AR,IC,S,N,IOLD,KREM,IER, STAT)
      IMPLICIT NONE
...
*
     ...
     & CR,CZ,H,S,R,EPS7,EPS9,GMAX,UMAX,KBF,KBC,INEW,IER,ITERM, STAT)
      IMPLICIT NONE
...
*
     ...
     &     GMAX,KBF,KBC,IOLD,KOLD,KREM,IER,ITERM, STAT)
      IMPLICIT NONE
...
c      COMMON /STAT/ NRES,NDEC,NREM,NADD,NIT,NFV,NFG,NFH
      ...
      CALL PLRMB0(NF,N,ICA,CG,CR,CZ,G,GN,IOLD,KREM,STAT(3),IER)
      IF (IER.LT.0) THEN
      ITERM=-16
