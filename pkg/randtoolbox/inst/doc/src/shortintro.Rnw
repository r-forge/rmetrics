...
Random simulation or Monte-Carlo methods rely on the fact we have access to random numbers. Even if nowadays having random sequence is no longer a problem, for many years producing random numbers was a big challenge. According to \cite{ripleybook}, simulation started in 1940s with physical devices. It brings us to a point we need to take care is the kind simulation we deal with. Using physical phenomena to get random numbers is referred in the literature as true randomness. However in our computers we use another type randomness: pseudo-randomness.

We define a sequence of pseudo-random numbers as a deterministic sequence of numbers in $[0,1]$ having the same relevant statistical properties as a sequence of random numbers. One may wonder with this definition how two calls to random number generator (RNG) can produce two different numbers. By default, a RNG is initialized by the machine time, i.e. the starting point is computed with the seconds, miliseconds of the machine clock. Then from this point (called the seed), we use deterministic algorithms to build the sequence.
...
\end{itemize}
Those properties seem obvious but \cite{mccullough08} proves that neither Excel 2003 nor Excel 2007 implements a RNG having the four properties. 

Of course in the statistical software \soft{R}, we have a RNG meeting the above properties called Mersenne-Twister. But this package aims to provide better and more recent RNGs as well as classic statistical tests to quantify their property. The rest of this chapter is two-folded: first we present the use of RNGs through the \code{runif} interface, second we present the same use with dedicated functions.
