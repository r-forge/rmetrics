...
theory. More about this may be found in the full presentation.

The best known random number generators are used for cryptographic purposes.
These generators are chosen so that
...
simulations, this requirement is usually relaxed. However, even for
...
statistics, which allow to distinguish bad generators from truly random
numbers.
