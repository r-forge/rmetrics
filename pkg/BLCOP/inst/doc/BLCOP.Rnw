...
\DefineVerbatimEnvironment{Code}{Verbatim}{commandchars=¬\{\}, frame = single, samepage = TRUE}
\begin{document}
...
stocks, 3 of which are technology stocks and the other 3 of which are from the financial sector.  Intially, she has a view on the technology sector and believes that the average of 2 of the stocks will outperform the third, say $\frac{1}{2}( \textrm{MS} + \textrm{IBM}) - \textrm{DELL} \sim N(0.06,  0.01)$.  We will create a 
\code{BLViews} class object with the \texttt{BLViews} constructor function.  Its arguments are the ``pick'' matrix, a vector of confidences, the vector ``q'', and the the names of the assets in one's ``universe''.  
...
Here we use \rfun{cov.shrink} from the \code{corpcov} package.
<<echo=TRUE,print=FALSE>>=
priorMeans <- rep(0, 6)
...
@
Now suppose that we wish to add another view, this time on the average of the three financial stocks.  This can be done
...
A user may also be interested in comparing allocations that are optimal under the prior and posterior distributions.  A helper function is provided
in the \texttt{BLCOP} package for doing this, and it can use any optimization routine that relies on a distribution's first and second moments.  This
helper function is \rfun{optimalPortfolios}.  If no optimizing function is passed in, it will use \rfun{solve.QP} to perform basic Markowitz optimization
...
Additional parameters may be passed into one's optimizer.  In the case of the default optimizer, one can pass in a \code{constraints} argument
that is used to control the execution of \rfun{solve.QP}.  Thus we can remove the ``long only'' constraint as in the following example:
...
@
Finally, the posterior is calculated with \rfun{COPPosterior}, and the updated marginal distributions can be visualized with \rfun{densityPlots}
...
as \code{P} in some places and \code{pick} in others) as well as improvements in the documentation and examples.  Aside from this, it would be natural to implement a version of the \rfun{optimalPortfolios} function for \code{COPResult} class objects, 
possibly based on CVaR optimization.  Attilio Meucci has also very recently proposed an even more general view-blending method which 
