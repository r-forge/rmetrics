...
#  solveRqpqc                Portfolio interface to QCQP solver Rsocp
#  .rqpqcArguments           Returns arguments for solver
#  .rqpqc                    Wrapper to solver function
#  .rqpqcPControl            Returns default controls for solver
################################################################################
...
{
    # Description:
    #   Portfolio interface to solve "Quadratic Constrained Quadratic
    #   Programming" QCQP problems with solver Rsocp
    ...
    #} else {
        ...
        args <- .rqpqcArguments(data, spec, constraints)
...
{
    # Description:
    #   Returns socp QCQP conform arguments for the solver
    ...
    #   QCQP:
    #
    #       minimize        x^T P_0 x + 2 q_0^T + r_0
    #       s.t.            x^t P_i x + 2 q_i^T + r_i  i=1, ...,p
    #
    #   equivalent to:
    #
    #       minimize        t
    #       s.t.   ||P_0^1/2 x + P_0^-1/2 q_0|| <= t
    #              ||P_i^1/2 x + P_i^-1/2 q_i|| <= q_i^T P_i^-1 q_i - r_i)^1/2  
    #                                 
    #
...
.rqpqc  <-
    ...
    targetRisk, mu = mu, control = .rqpqcControl())
{
    # Description:
    #   SOCP solver function for QPQC portfolios
