...
qqskewhyp <- function(y, mu = 0, delta = 1, beta = 1, nu = 1,
                      ...
                      main="Skewed Hyperbolic Student's-t QQ Plot",
                      xlab="Theoretical Quantiles",
                      ylab="Sample Quantiles",
                      plot.it = TRUE, line = TRUE,...){

    if (has.na <- any(ina <- is.na(y))) {#how does that work
        yN <- y
        y <- y[!ina]
    }
    if (0 == (n <- length(y))){
    ...
    } else { ###change start values method####
...
ppskewhyp <- function(y, beta = NULL, delta = NULL, mu = NULL, nu = NULL,
                      param = c(mu,delta,beta,nu),
                      main = "Skewed Hyperbolic Student's-t P-P Plot",
