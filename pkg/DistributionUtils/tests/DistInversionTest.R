...
inversionTestpq <- function(densFn = "norm", n = 10, param = NULL,
                            ...
                            uniTol = intTol, x = NULL, method = "spline", ...)
{
    ...
    qpx <- qDist(densFn, p = pDist(densFn, q = x,
                         param = param, intTol = intTol, ...),
                 param = param, uniTol = uniTol, method = "spline", ...)
    diffs <- qpx - x
...
{
    CALL <- match.call()
    q <- qDist(densFn, p = qs, param = param, uniTol = uniTol, ...)
    pqqs <- pDist(densFn, q = q, param = param, intTol = intTol, ...)
    diffs <- pqqs - qs
