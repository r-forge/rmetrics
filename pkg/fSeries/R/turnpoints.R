...
#  turnpoints                 Returns peaks and pits from a timeSeries
    ...
    function(x, ...)
{
    ...
    # Example:
    #   .turnpoints(as.timeSeries(data(msft.dat))[, 4])
    ...
    function(x) 
{
    ...
    cat("E(p) =", 2 / 3 * (x$n - 2), "Var(p) =", (16 * x$n - 29) / 90, 
        "(theoretical)\n")
    cat("\n")
    
    # construct the table summarizing all turning points
    typepts <- rep(typep, length.out=x$nturns)
    tablepts <- as.data.frame(list(point = x$tppos, type = typepts, 
        proba = x$proba, info = x$info))
    print(tablepts)
