...
function()
{
    # Help File:
    helpFile = function() { 
        example(GpdModelling, ask = FALSE)
        return() 
    }
    checkIdentical(
...
function()
{
    ...
    # PWM Fit:
    fit = gpdFit(tS, u = min(tS@Data), "pwm") 
    print(fit)
    ...
    # MLE Fit:
    fit = gpdFit(tS, u = min(tS@Data), "mle") 
    print(fit)
    ...
    # Information:
    fit = gpdFit(tS, u = min(tS@Data), type = "mle", information = "observed") 
    print(fit)
...
if (FALSE) {
    require(RUnit)
    testResult <- runTestFile("C:/Rmetrics/SVN/trunk/fExtremes/tests/runit3B.R",
