    ...
    sigf = 12, maxiter = 100, margin = 1e-8, bound = 10, verb = 0)
{
    ...
    if(smw == 0) {
        ...
        s.tmp <- solve(AP, c(c.x, c.y), tol = .Machine$double.eps, 
            LINPACK = TRUE)
        x <- s.tmp[1:n]
        y <- s.tmp[-(1:n)]
    } else {
        ...
        y <- solve(A %*% smwa2 + H.y , c.y + A %*% smwc2, 
            tol = .Machine$double.eps, LINPACK = TRUE)
        x <- smwa2 %*% y - smwc2
    }
    ...
    while (counter < maxiter) {
        ...
        if(smw == 0) {
            ...
            smwa2 <- t(A) - (H %*% solve(smwinner,solve(t(smwinner),
                crossprod(H,smwa1)), tol = .Machine$double.eps, 
                LINPACK = TRUE))
            smwa2 <- smwa2 / d 
            smwc2 <- (c.x - (H %*% solve(smwinner,solve(t(smwinner),
                crossprod(H,smwc1)), tol = .Machine$double.eps, 
                LINPACK = TRUE)))/d 
