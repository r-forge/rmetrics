...
{
  list(iterma=as.integer(iterma), nstep=as.integer(nstep),
       fnscale=fnscale,report=report,
       rep.freq=as.integer(ifelse(rep.freq<1, 1, rep.freq)),

       tau0=tau0,tau=tau,del0=del0,

       epsx=epsx,delmin=delmin,epsdif=epsdif,
       ...
       te0=te0,te1=te1,te2=te2,te3=te3,
       silent=silent, intakt=intakt)
}
                   ...
                   env=.GlobalEnv, name=NULL)
{
  ...
  tryCatch(
           ...
           ans <- .Call("call_donlp2",
                        ...
                        control,
                        accfun,
                        confun, environment(confun), PACKAGE="fPortfolio"),
           ## ensure to free memory and close .mes .pro files if opened
           finally=.Call("teardown", 0, PACKAGE="fPortfolio"))
