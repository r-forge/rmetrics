...
kurtosis.default <- function (x, na.rm = FALSE, method = c("excess", "moment", "fisher"), ...) {
    ...
    if (method == "fisher") {
        kurtosis <- ((n+1)*(n-1)*((sum(x^4)/n)/(sum(x^2)/n)^2 - (3*(n-1))/(n+1))) /
            ((n-2)*(n-3))
    }
...
kurtosis.data.frame <- function (x, na.rm = FALSE, method = c("excess", "moment", "fisher"), ...) {
    ## A function implemented by Diethelm Wuertz
...
kurtosis.POSIXct <- function (x, ...) {
    ## A function implemented by Diethelm Wuertz

    structure(kurtosis(unclass(x), ...), class = oldClass(x))
}

kurtosis.POSIXlt <- function (x, ...) {
    ## A function implemented by Diethelm Wuertz
