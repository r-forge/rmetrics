...
",file = dstFile)
    ...
    for (k in seq(length(finCenter))) {
        ...
        zdump <- try(system(paste("zdump -v", finCenter[k], sep=" "),
                            intern=TRUE))
        ...
        gmtoff <- as.integer(substr(zdump[,16],8,12))
        ...
        # construct DST table rule for fin center
