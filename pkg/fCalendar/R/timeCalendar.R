...
.monthlySequence = function(from = NULL, to = .today(), length = 12)
{
    # Example:
    #   .monthlySequence(); .monthlySequence(from = today()-366*24*3600)

    if (is.null(from)) {
        to.year = as.POSIXlt(to)$year+1900
        to.month = as.POSIXlt(to)$mon+1 
        from.year = (to.year * 12 + to.month - length) %/% 12
        from.month = (to.year * 12 + to.month - length) %% 12
        to = to.year*10000 + to.month*100 + 1
        from = from.year*10000 + from.month*100 + 1
    }
    
    tS = timeSequence(from, to, by = "month")      
...
.quarterlySequence = function(from = NULL, to = .today(), length = 4)
{
    # Example:
    #   .quarterlySequence(); .quarterlySequence(from = today()-366*24*3600)
    ...
    tS = timeSequence(from, to, by = "month")      
    unique(timeLastDayInQuarter(tS))
}
