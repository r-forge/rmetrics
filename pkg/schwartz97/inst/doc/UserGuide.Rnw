...
days and deltat is set to 1/52 because weekly price observations are
used here.

First, data have to be made weekly. Wednesday observations are
...
\caption{Real and fitted term structures of soybean meal
  data.}\label{fig:term-structure-soybean}
\end{figure}
