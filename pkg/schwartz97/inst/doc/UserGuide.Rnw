...
In the next example we draw a trajectory from the state variables,
i.e., of the spot price and the convenience yield. The spot price and
the convenience yield are simulated on a daily basis over the next
five years using \verb=simstate=. Then, a sample of the spot price and
the convenience yield in five years is generated by the function
