...
closed-form solutions can be obtained for both, approximate value at
risk (VaR) and approximate expected shortfall (ES)\footnote{It is an
  approximation in the sense that the calculations are based on
  \textit{log}-returns.}.
