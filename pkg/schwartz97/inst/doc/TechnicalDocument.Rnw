...
\begin{abstract}
  ...
  needed to understand the Schwartz two-factor model
  \citep{Schwartz1997}. This includes parameter estimation using the
  Kalman filter as well as pricing of European options according to
  \cite{Miltersen1998}.
\end{abstract}
...
% <============================================================>
This document will describe the Schwartz two-factor model to the
extent which is necessary to understand the \RR{} package \Rpackage.  The
two-factor model describes the spot price of a commodity as well as
its instantaneous convenience yield. It was introduced in
\cite{Gibson1990} and extended in \cite{Schwartz1997} for the pricing
of futures contracts. \cite{Miltersen1998} presented equations for
arbitrage free prices of European options on commodity futures. In
particular, they provided a formula for the three-factor model (which
includes a stochastic interest rate). In what follows we fully rely on
the above mentioned articles and state the corresponding formulas.
% <============================================================>
...
Under the equivalent martingale measure $\mathbb{Q}$ the dynamics are
of the form
\begin{eqnarray}
...
normally distributed (cf. appendix \ref{sec:deriv-joint-distr} for a
derivation):
\begin{equation}
\label{jointDist}
\begin{pmatrix}X_t \\ \delta_t \end{pmatrix} \sim
\mathcal{N}\left( \boldsymbol{\mu} = \begin{pmatrix}\mu_X \\
    \mu_\delta \end{pmatrix},
  \boldsymbol{\Sigma} = \begin{pmatrix}\sigma_X^2 & \sigma_{X\delta}  \\ \sigma_{X\delta} & \sigma_\delta^2  \\
    \end{pmatrix} \right),
\end{equation}
with parameters
\begin{align}
  \mu_X &= X_0 + \left(\mu - \frac{1}{2}\sigmaS^2 - \alpha\right) t
  + \left(\alpha  - \delta_0\right) \frac{1 - e^{-\kappa t}}{\kappa}\label{eq:spot-mu}\\
  \mu_\delta &= e^{-\kappa t}\delta_0 + \alpha \left(1 - e^{-\kappa
      t}\right)\label{eq:cy-mu}\\
  \sigma_X^2 &= \frac{\sigmaE^2}{\kappa^2}
  ...
  \sigma_\delta^2 &= \frac{\sigmaE^2}{2\kappa}\left(1 -
    e^{-2\kappa t}\right) \label{eq:cy-var}\\
  \sigma_{X\delta} &= \frac{1}{\kappa}\left\{\left(\sigmaS\sigmaE\rho -
      \frac{\sigmaE^2}{\kappa}\right)\left(1 - e^{-\kappa t}\right) +
    \frac{\sigmaE^2}{2\kappa}\left(1 - e^{-2\kappa
        t}\right)\right\}\label{eq:spot-cov}
\end{align}
The mean-parameters given in equations \ref{eq:spot-mu} and
...
the $\mathbb{Q}$-parameters be denoted by $\tilde{\mu}_X$ and
$\tilde{\mu}_{\delta}$.
% <============================================================>
...
Let the futures price at time $t$ with time to maturity $\tau = (T -
t)$ be $G(S_t, \delta_t, T - t)$. A futures contract must be priced
under the spot martingale measure $\mathbb{Q}$. Let's start at time
zero:
\begin{equation}
  \label{futuresPrice}
  G(S_0, \delta_0, T) = \mathbb{E}^{\mathbb{Q}}(S_T)
\end{equation}
From the previous section \ref{sec:distributions} we know that
$\log(S_T)$ is a Gaussian random variable. Hence we can easily
calculate the futures price by taking the expectation under the
measure $\mathbb{Q}$ (i.e. using $\tilde{\mu}_X$ and
$\tilde{\mu}_{\delta}$).
\begin{eqnarray}
  \label{futuresPrice}
  G(S_0, \delta_0, T) &=& \exp\left\{\tilde{\mu}_X + \frac{1}{2}\sigma_X^2\right\}\\
  &=& S_0 e^{A(T) + B(T)\delta_0}
\end{eqnarray}
with
\begin{align}
  A(\tau) &= \left(r - \tilde{\alpha} + \frac{1}{2}
    \frac{\sigmaE^2}{\kappa^2} - \frac{\sigmaS \sigmaE
      \rho}{\kappa}\right) \tau  + \frac{1}{4} \sigmaE^2 \frac{1 - e^{-
      2 \kappa \tau}}{\kappa^3} + \left( \kappa\tilde{\alpha} +
    \sigmaS \sigmaE \rho - \frac{\sigmaE^2}{\kappa} \right) \frac{1 -
    e^{-\kappa \tau}}{\kappa^2}\label{eq:A-futures},\\
  B(\tau) &= - \frac{1 - e^{-\kappa \tau}}{\kappa}\label{eq:B-futures}.
\end{align}
% <============================================================>
\subsection{Distribution of Futures Prices}
% <============================================================>
In the Schwartz model the log-price of a futures contract is a linear
combination of the models normally distributed state variables and is
hence also normally distributed, i.e.,
\begin{align}
\log{G(t,T)} &\sim \mathcal{N}\left(\mu_G, \sigma_G^2\right),\label{distrG}
\end{align}
where $t$ stands for the time the stochastic processes $\log{G}$ is
evaluated and $T$ denotes the maturity date.

According to (\ref{jointDist}) and (\ref{futuresPrice}) the parameters
for the distribution (\ref{distrG}) of the futures price become
\begin{align}
  \mu_G &= \mu_X + A(T-t) + B(T-t)\mu_\delta \label{muG} \\
  \sigma_G^2 &= \sigma_X^2 + 2 B(T-t)\sigma_{X\delta} + +
  B(T-t)^2 \sigma_\delta. \label{varG}
\end{align}



% <============================================================>
\section{Risk Measures for Futures Contracts}
\label{sec:Risk}
% <============================================================>
The log-returns of futures contracts are normally distributed since in
our model the futures prices are log-normal. Using this fact,
closed-form solutions can be obtained for both, value at risk (VaR)
and expected shortfall (ES).

At time zero and considering a time period $\Delta t$, the
\textit{negative} log-return or loss $l$ of a futures contract with
maturity $T$ is defined to be
\begin{equation}
  l(\Delta t, T) := \log{G(0,T)} - \log{G(\Delta t,T)}
\end{equation}

According to (\ref{distrG}) -- (\ref{varG}) the potential loss during
time period $\Delta t$ is given by
\begin{equation}
  \label{distrLoss}
  l(\Delta t,T) \sim \mathcal{N}\left(\log{G(0,T)}-\mu_G(\Delta
  t,T),\sigma_G^2(\Delta t,T)\right).
\end{equation}

The VaR for the normally distributed negative log-return $l(\Delta
t,T)$ times the futures price at time zero $G(0,T)$ gives us the VaR
for the futures contract. The VaR at the confidence level
$\alpha$\footnote{For a 95\%-VaR one sets $\alpha = 0.95$.} for the
futures contract $G(0,T)$ at time zero thus becomes
\begin{equation}
  \label{VaRG}
  VaR_\alpha^G = G(0,T) \left(\log{G(0,T)} - \mu_G +
  \sigma_G\Phi^{-1}\left(\alpha\right)\right)
\end{equation}
where $\Phi$ denotes the standard normal distribution function and
$\Phi^{-1}(\alpha)$ is the $\alpha$-quantile of $\Phi$.

Similarly the ES at the confidence level $\alpha$ for the futures
contract $G(0,T)$ at time is then given by
\begin{equation}
  \label{ESG}
  ES_\alpha^G = G(0,T)\left(\log{G(0,T)} - \mu_G +
    \sigma_G\frac{\phi\left(\Phi^{-1}(\alpha)\right)}{1 -
      \alpha}\right)
\end{equation}
...
contract was derived in \cite{Miltersen1998} for a general setting,
and in particular for the models presented in \cite{Schwartz1997}.
...
\begin{align}
  C &= \mathbb{E}^{\mathbb{Q}}\left[e^{-r\,t}(G(S_t, \delta_t, T - t)
    - K)^+\right]
\end{align}
...
\begin{equation}
C^G - P^G = P(0,t)\left\{G(0,t)e^\alpha - K\right\}.
\end{equation}
...
a commodity \textit{spot} contract.


% <============================================================>
\section{Calibration}\label{sec:Calibration}
% <============================================================>
This section demonstrates an elegant way of calibrating the Schwartz
multi-factor model. That is estimating the model parameters by
applying the Kalman filter methodology. Subsection
\ref{sec:stateSpace} contains a documentation of how the Schwartz
two-factor model can be expressed in state space form. Once the model
has been cast in this form one can compute MLE estimates of arbitrary
model parameters by numerically maximizing the Kalman-filter
likelihood function.
% <============================================================>
\subsection{State Space Representation}
\label{sec:stateSpace}
% <============================================================>
Let $\mathbf{y}_t$ denote a $(n\times{1})$ vector of variables
observed at date $t$ and $\boldsymbol{\alpha}_t$ denote a
$(2\times{1})$ vector named \textit{state vector}. The \textit{state
  space representation} of the dynamics of $\mathbf{y}$ may be
given by the linear system of equations
\begin{eqnarray}
...
(``innovations'') in the measurement equation (\ref{eq:measurement}) are
further assumed to be independent, i.e.
\begin{equation}
  \label{eq:1}
...
eq. (\ref{eq:A-futures}) and (\ref{eq:B-futures}) the components of
the state space representation (eq. \ref{eq:measurement} and
eq. \ref{eq:transition}) are
\begin{eqnarray}
...
the \textit{remaining time to maturity} of the $i$-th closest to
...
\end{eqnarray}
Let's have a look on the integral $\int_0^t \delta_u du$.
