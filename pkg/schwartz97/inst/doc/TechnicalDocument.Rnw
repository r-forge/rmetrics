...
\begin{equation}
  \label{VaRG}
  VaR_\alpha^G = \left(\log{G(0,T)} - \mu_G +
...
\begin{equation}
  \label{ESG}
  ES_\alpha^G = \left(\log{G(0,T)} - \mu_G +
...
\begin{equation}
  \label{ESG}
  ES_\alpha^G = \frac{1}{1 - \alpha}e^{\mu_G +
    \frac{1}{2}\sigma_G^2}\left[1 -
    \Phi\left(\Phi^{-1}(\alpha)-\sigma_G\right)\right]
\end{equation}
where $\phi$ is the density of the standard normal distribution.
