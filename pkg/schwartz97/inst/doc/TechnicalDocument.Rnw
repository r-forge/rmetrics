...
\begin{abstract}
  ...
  Kalman filter, pricing of European options according to
  \cite{Miltersen1998} as well as computation of risk measures.
\end{abstract}
...
extent which is necessary to understand the \RR{} package \Rpackage.
The two-factor model describes the spot price of a commodity together
with its instantaneous convenience yield. It was introduced in
...
what follows we fully rely on the above mentioned articles and state
the corresponding formulas. [MENTION CLOSE FORM ETC, dW HAS TO BE IN LINE]
% <============================================================>
...
% <============================================================>
The log-spot $X_t = \log(S_t)$ and the convenience yield $\delta_t$ are jointly
normally distributed (see appendix \ref{sec:deriv-joint-distr} for a
derivation):
\begin{equation}
\label{jointDist}
...
be $G(S_t, \delta_t, t, T)$. At time zero the futures price is given
by the $\mathbb{Q}$-expectation of $S_T$.
\begin{eqnarray}
...
log-normally law. That is, at time zero the $T$-futures price at time
$t$ has distribution
\begin{align}
...
where
\begin{align}
  \mu_G(t, T) &= \mu_X(t) + A(T-t) + B(T-t)\mu_\delta(t) \label{eq:muG} \\
...
\label{sec:Risk}
% <============================================================>
At time zero the log-loss over a time period $\Delta t$ is defined to
be
\begin{equation}
  l(\Delta t, T) := \log{G(S_0, \delta_0, 0,T)} - \log{G(S_{\Delta t},
    \delta_{\Delta t}, \Delta t,T)}
\end{equation}

According to (\ref{eq:distrG}) -- (\ref{eq:varG}) the VaR for the is
therefore
\begin{equation}
  \label{VaRG}
  VaR_\alpha^G = \log{G(S_0, \delta_0, 0,T)} - \mu_G( +
