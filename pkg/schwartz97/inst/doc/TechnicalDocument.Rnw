...
\author{Philipp Erb, David L\"{u}thi \& Simon Otziger}%\\\email{philipp.erb@zhaw.org}}
\date{\today}
\begin{document}
\maketitle
\begin{abstract}
...
% <============================================================>
[See also \ref{U-sec:intro}.]

This document will describe the Schwartz
two-factor model to the extent which is necessary to understand the
\RR{} package \Rpackage.  The two-factor model describes the spot
price of a commodity as well as its instantaneous convenience
yield. It was introduced in \cite{Gibson1990} and extended in
\cite{Schwartz1997} for the pricing of futures
contracts. \cite{Miltersen1998} presented equations for arbitrage free
prices of European options on commodity futures. In particular, they
provided a formula for the three-factor model (which includes a
stochastic interest rate). In what follows we fully rely on the above
mentioned articles and state the corresponding formulas.
% <============================================================>
...
\textit{negative} log-return or loss $l$ of a futures price with
...
\subsection{Log-Returns of Futures Price}
The VaR for the normally distributed negative log-return $l(\Delta
t,T)$ at confidence level $\alpha$\footnote{For a 95\%-VaR one sets
  $\alpha = 0.95$.} becomes
\begin{equation}
  \label{VaRG}
  VaR_\alpha^G = \log{G(0,T)} - \mu_G +
  \sigma_G\Phi^{-1}\left(\alpha\right)
\end{equation}
where $\Phi$ denotes the standard normal distribution function and
$\Phi^{-1}(\alpha)$ is the $\alpha$-quantile of $\Phi$.

Similarly the ES for the negative log-return at the confidence level
$\alpha$ is given by
\begin{equation}
  \label{ESG}
  ES_\alpha^G = \log{G(0,T)} - \mu_G +
    \sigma_G\frac{\phi\left(\Phi^{-1}(\alpha)\right)}{1 -
      \alpha}
\end{equation}
where $\phi$ is the density of the standard normal distribution.

\subsection{Nominal Futures Price}
The VaR for the log-normally distributed futures price $G(\Delta
t,T)$ at confidence level $\alpha$\footnote{For a 95\%-VaR one sets
  $\alpha = 0.05$.} becomes
\begin{equation}
  \label{VaRG}
  VaR_\alpha^G = e^{\mu_G +
    \frac{1}{2}\sigma_G^2} e^{\sigma_G\left(\Phi^{-1}(\alpha) -
      \frac{\sigma_G}{2}\right)}
\end{equation}
where $\Phi$ denotes the standard normal distribution function and
$\Phi^{-1}(\alpha)$ is the $\alpha$-quantile of $\Phi$.

Similarly the ES for the futures price at the confidence level
$\alpha$ is given by
\begin{equation}
  \label{ESG}
  ES_\alpha^G = \frac{1}{\alpha}e^{\mu_G +
    \frac{1}{2}\sigma_G^2}\Phi\left(\Phi^{-1}(\alpha) -
      \sigma_G\right)
\end{equation}
where $\phi$ is the density of the standard normal distribution.
...
has been cast in this form one can compute maximum likelihood
estimates of arbitrary model parameters by numerically maximizing the
