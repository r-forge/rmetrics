...
parameter fitting routine \verb=fit.schwartz2f=. Once the parameters of
...
\begin{center}
  \begin{tabular}{l|l}
    ...
    \verb=filter.schwartz2f= & Filter a futures-price time-series. \\
    \verb=fit.schwartz2f= & Fit the two-factor model to data. \\
  \end{tabular}
\end{center}
\vspace{3mm} Except the function \verb=fit.schwartz2f=, all the above
...
\verb=schwartz2f= the constructor with the same name can be used
(see section \ref{sec:init}).

The function \verb=fit.schwartz2f=, which fits the two-factor model to
data, returns an object of class \verb=schwartz2f.fit=. This
class inherits from the class \verb=schwartz2f= and adds around
...
extensions (to class \verb=schwartz2f=) by the class
\verb=schwartz2f.fit= refer to parameters which are needed to
...
(type \verb=?schwartz2f.fit-class=
...
\ref{sec:functions} (except \verb=fit.schwartz2f=) are set to generic.
...
If the parameters are estimated by the function \verb=fit.schwartz2f=, we
...
\verb=schwartz2f= objects with different \verb=s0= and
...
% <============================================================>
As soon as a \verb=schwartz2f= object is constructed it can be
...
estimated by the function \verb=fit.schwartz2f= (see section
...
copper.fit <- fit.schwartz2f(copper$futures[!na.idx,], copper$ttm[!na.idx,],
...
convenience yield.
<<>>=
filtered <- filter.schwartz2f(copper$futures[!na.idx,], copper$ttm[!na.idx,], copper.fit)
@
\begin{figure}\centering
<<fig=TRUE,echo=FALSE>>=
par(mfrow = c(2, 1))
