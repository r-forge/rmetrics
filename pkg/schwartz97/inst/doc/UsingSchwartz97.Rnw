...
approach follwed in this package is explained.
% <------------------------------------------------------------>
...
extensions by the class \verb=fit.schwartz2factor= refer to parameters
which are needed to work with derivatives (as the market price of
convenience yield risk parameter $\lambda$). The following table
describes these slots. A complete description of the slots can be
found in the R-documentation (type \verb=?fit.schwartz2factor-class=
...
The idea is to leave some freedom to the user, which can decide
...
dfutures(x, ttm = 1, s0, r = 0.05, lambda = 0, alphaT = NULL, ...)
...
constructed in the following code junk.
<<>>=
library(schwartz97)
s0 <- 100
delta0 <- 0
...
state variables for a given point of time in the future, once this
...
r <- 0.03
lambda <- 0
futures.trajectory <- pricefutures(ttm =  ttm, state.trajectory[, "S"],
                                   state.trajectory[,"delta"],
...
\caption{The top figure shows the trajectory of the spot price
  ...
  the spot price. Such a situation is called
  backwardation.}\label{fig:futures-trajectory}
\end{figure}
% <============================================================>
...
a first glance, it is difficult to assign a value to a parameter which
enters a stochstic differential equation of a non-observable process
...
30\%. The convenience yield is expected to decrease fastly to its
long-term average. ``Decrease fastly'' is reflected in a $\kappa$ of
...
in one year. The following code-junk shows how the arbitrage-free
...
involved in equations \ref{eq:Pdynamics-S} - \ref{eq:Qdynamics-delta}
(including the initial values of the state variables $s_0$ and
$\delta_0$). The estimation procedure is based on the Kalman filter.
Because the log-futures prices linearly depend on two Gaussian random
variables (the log-spot price and the convenience yield), it is the
most straightforward way to use a linear state-space model.
...
stating which parameters shall be fitted. The argument
...
long-term average parameter \verb=alpha= and the volatility parameter
