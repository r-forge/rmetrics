...
% <---------------------------------------------------------------------->
\title{
ghyp: A package on generalized hyperbolic distributions\\[10ex]
}
\author{
Wolfgang Breymann\\
David L\"uthi\\[3ex]
...
overlaps McNeil's library QRMlib \citep{QRMlib}.  However, there are
...
multivariate GH distributions is \citet*{QRMmcneil05}, where one can
...
according to \citet*{QRMmcneil05} chapter $3.2$.\\[1ex]
% <---------------------------------------------------------------------->
...
\begin{proposition}
  ...
  \mathbb{R}^{k \times d}$ and $\mathbf{b} \in \mathbb{R}^k$, then $Y
  \sim \GHtransf$.
\end{proposition}
\begin{proof}
...
\psi, \GAMMA$) become location and scale-invariant.
% <---------------------------------------------------------------------->
...
\begin{table}[h]
  ...
  \caption{The domain of variation for the parameters of the GH distribution and
    ...
    matrices in $\mathbb{R}^{d \times d}$ with $\mathbb{R}^\Sigma$.
    Furthermore, let $\mathbb{R}^\Delta = \{A \inRealSigma : \dete{A} = 1\}$.}\label{tab:param}
\end{table}
